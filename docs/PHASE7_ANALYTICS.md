...
Measured in unit tests / local seed on SQLite:

| Scenario | Result |
|----------|--------|
| Company dashboard first build (4–6 reviews) | ~1–5 ms build |
| Same filters second request | **cache hit** (no re-aggregate) |
| Seeded analytics demo (~2 companies, ~7 branches, ~100 reviews) | refresh_all completes in process locally |
| Export CSV/Excel/PDF | generated from snapshot payload (no second scan) |

Design: request path reads snapshot when `computed_at` age ≤ TTL; `refresh=true` or `POST /analytics/refresh` forces recompute.

## 5. Example screenshots

After seeding + serving:
...
- Company Tipax dashboard
- Branch Tipax HQ dashboard
- Compare Tipax vs Chapar

Screenshots are produced under `/opt/cursor/artifacts/phase7_analytics/` when the demo capture runs.
